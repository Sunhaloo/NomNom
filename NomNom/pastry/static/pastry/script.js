$(function () {
  ...
  function loadProductReviews(productName) {
    ...
    const productCard = document.querySelector(
      `.product-card[data-product="${productName}"]`,
    );
    if (!productCard) {
      ...
      .done(function (data) {
        if (data.reviews) {
          ...
          if (data.reviews.length > 0) {
            totalRating = data.reviews.reduce(
          ...
          if (data.reviews.length === 0) {
            $reviewList.html(
              '<div class="review-item">No reviews yet. Be the first reviewer!</div>',
            );
          } else {
  ...
  if (reviewForm) {
    reviewForm.addEventListener("submit", function (e) {
      ...
      if (!isAuthenticated) {
        showLoginPrompt(
          "You must login and purchase this item to submit a review.",
        );
      ...
      const pastryName = currentProductName;

      // Get the pastry ID by searching the page for it
      const productCard = document.querySelector(
        `.product-card[data-product="${pastryName}"]`,
      );
      if (!productCard) {
  ...
  function showNotification(message, type = "success") {
    ...
    if (type === "error") {
      ...
      $notification.css("background-color", "#5D4037");
    }
